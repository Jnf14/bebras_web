...
def draw(axiom, d=D, l=L, x=0, y=0, show_plug_points=False):
    ...
    myturtle.penup()
    myturtle.goto(x, y)
    myturtle.setheading(90)    # Point up instead of right
    myturtle.pendown()
    myturtle.color("red")
    for i in range(4):
        myturtle.fd(20)
...
draw(result,angle,len_list[i],pos_list[i][0],pos_list[i][1])
i = i + 1
#---
result = iterate(rule, 1, result)
draw(result,angle,len_list[i],pos_list[i][0],pos_list[i][1])
i = i + 1
#---
result = iterate(rule, 1, result)
